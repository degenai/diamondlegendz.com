...
export function startCharge(p) {
  if (p.chargeT >= 0 || p.palmT > 0 || p.lungeT > 0 || p.knockedT > 0 || p.foldT > 0 || p.vehicle || p.massaging || p.counterArmed) return false;   // an armed Q counter (goon-counter.js) goes first
